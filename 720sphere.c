...
void sphereColor(const void *body, const rayQuery *query, 
	...
	if (recursionNum > 0) {
		double rgbResult[3];
		double cTran[3] = {0.4, 0.4, 0.4};
		
		double airIndex = 1.0;
		double glassIndex = 1.2;
		double dRefr[3], dInc[3];
		vecCopy(3, dCameraLocal, dInc);
		rayRefraction(dNormalLocal, airIndex, dInc, glassIndex, dRefr);

		rayQuery transQuery;
		transQuery.tStart = rayEPSILON;
		transQuery.tEnd = rayINFINITY;
		vecCopy(3, xWorld, transQuery.e);
		isoRotateVector(&(sphere)->isometry, dRefr, transQuery.d);
		// printf("dRefr: %f %f %f\n", dRefr[0], dRefr[1], dRefr[2]);
		rayColor(bodyNum, bodies, lightNum, lights, cAmbient, &transQuery, recursionNum - 1, rgbResult);
		rgb[0] += rgbResult[0] * cTran[0];
		rgb[1] += rgbResult[1] * cTran[1];
		rgb[2] += rgbResult[2] * cTran[2];
