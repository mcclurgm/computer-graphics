...
void texSetFiltering(texTexture *tex, int filtering) {
	...
	if(filtering == 0) {
	    printf("Linear filtering (0)\n");
	} else {
	    printf("Nearest neighbor filtering (1)\n");
	}
}
...
void texSample(const texTexture *tex, double s, double t, double sample[]) {
	...
	int i;
	if (tex->filtering == texLINEAR) { // linear filtering
	    double fracU = u - floor(u);
	    double fracV = v - floor(v);
	    int dim = tex->texelDim;
	    
	    // Get colors at corners of pixel
	    double bottomLeft[dim], bottomRight[dim], topLeft[dim], topRight[dim];
		texGetTexel(tex, (int)floor(u), (int)floor(v), bottomLeft); // bottom left
		texGetTexel(tex, (int)floor(u), (int)ceil(v), bottomRight); // bottom right
		texGetTexel(tex, (int)ceil(u), (int)floor(v), topLeft); // top left
		texGetTexel(tex, (int)ceil(u), (int)ceil(v), topRight); // top right
		
		double factorBL = (1 - fracU) * (1 - fracV);
		double factorBR = fracU * (1 - fracV);
		double factorTL = (1 - fracU) * fracV;
		double factorTR = (fracU) * (fracV);
		
		// Scale colors by their factors
		double scaledBL[dim], scaledBR[dim], scaledTL[dim], scaledTR[dim];
		vecScale(tex->texelDim, factorBL, bottomLeft, scaledBL);
		vecScale(tex->texelDim, factorBR, bottomRight, scaledBR);
		vecScale(tex->texelDim, factorTL, topLeft, scaledTL);
		vecScale(tex->texelDim, factorTR, topRight, scaledTR);
		
		// Add vectors to get final color
		double bottom[dim], top[dim];
		vecAdd(tex->texelDim, scaledBL, scaledBR, bottom);
		vecAdd(tex->texelDim, scaledTL, scaledTR, top);
		vecAdd(tex->texelDim, bottom, top, sample);
