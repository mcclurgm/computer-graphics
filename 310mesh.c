...
backing resources. */
GLuint meshInitialize(meshMesh *mesh, GLuint triNum, GLuint vertNum, 
		GLuint attrDim) {
...
GLuint *meshGetTrianglePointer(const meshMesh *mesh, GLuint tri) {
	if (tri < mesh->triNum)
...
void meshSetVertex(meshMesh *mesh, GLuint vert, const GLdouble attr[]) {
	GLuint k;
...
GLdouble *meshGetVertexPointer(const meshMesh *mesh, GLuint vert) {
	if (vert < mesh->vertNum)
...
GLuint meshFileError(meshMesh *mesh, FILE *file, const char *cause, 
		const GLuint line) {
...
GLuint meshInitializeFile(meshMesh *mesh, const char *path) {
	FILE *file = fopen(path, "r");
	if (file == NULL) {
		fprintf(stderr, "error: meshInitializeFile: fopen failed\n");
		return 1;
	}
	GLuint year, month, day, triNum, vertNum, attrDim;
	...
	for (line = 6; line < triNum + 6; line += 1) {
		...
		if (tri[0] >= vertNum || tri[1] >= vertNum || tri[2] >= vertNum)
...
where the latter is an GLuinteger value. The third and fourth lines do the same 
for vertNum and attrDim. The fifth line says '[triNum] Triangles:'. Then there 
are triNum lines, each holding three GLuintegers between 0 and vertNum - 1 
(separated by a space). Then there is a line that says '[vertNum] Vertices:'. 
Then there are vertNum lines, each holding attrDim floating-poGLuint numbers 
(terminated by a space). */
GLuint meshSaveFile(const meshMesh *mesh, const char *path) {
...
GLuint meshInitializeRectangle(meshMesh *mesh, GLdouble left, GLdouble right, 
...
GLuint meshInitializeEllipse(meshMesh *mesh, GLdouble x, GLdouble y, 
...
triangle. Computes the outward-poGLuinting unit normal vector for the triangle. 
The output CANNOT safely alias the input. */
void meshTrueNormal(const GLdouble a[], const GLdouble b[], const GLdouble c[], 
		GLdouble normal[3]) {
...
GLuint meshInitializeBox(meshMesh *mesh, GLdouble left, GLdouble right, 
...
the input. */
void meshRotateVector(GLdouble theta, const GLdouble v[2], GLdouble vRot[2]) {
...
finished. */
GLuint meshInitializeRevolution(meshMesh *mesh, GLuint zNum, 
		const GLdouble z[], const GLdouble r[], const GLdouble t[], 
		GLuint sideNum) {
	GLuint i, j, error;
...
GLuint meshInitializeSphere(meshMesh *mesh, GLdouble r, GLuint layerNum, 
		GLuint sideNum) {
	GLuint error, i;
...
when finished. */
GLuint meshInitializeCapsule(meshMesh *mesh, GLdouble r, GLdouble l, 
		GLuint layerNum, GLuint sideNum) {
	GLuint error, i;
	GLdouble theta;
	GLdouble *ts = (GLdouble *)malloc((2 * layerNum + 2) * 3 * sizeof(GLdouble));
	if (ts == NULL)
		return 1;
	else {
...
GLdouble zs[3][4] = {
	...
	{6.0, 5.0, 3.0, 1.0}, 
	{4.0, 3.0, -1.0, -2.0}};
GLuint error = meshInitializeLandscape(&mesh, 3, 4, 20.0, (GLdouble *)zs); */
GLuint meshInitializeLandscape(meshMesh *mesh, GLuint width, GLuint height, 
		GLdouble spacing, const GLdouble *data) {
	GLuint i, j, error;
	GLuint a, b, c, d;
		...
		for (i = 0; i < width; i += 1)
			for (j = 0; j < height; j += 1) {
				...
				vec8Set(i * spacing, j * spacing, data[i * height + j], 
					(GLdouble)i, (GLdouble)j, 0.0, 0.0, 0.0, vert);
			}
		/* Build the triangles. */
...
GLuint meshInitializeDissectedLandscape(meshMesh *mesh, const meshMesh *land, 
