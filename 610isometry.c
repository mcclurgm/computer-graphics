...
void isoUntransformPoint(const isoIsometry *iso, const double isoP[3], double p[3]) {
	double subtracted[3];
    vecSubtract(3, isoP, iso->translation, subtracted);
    mat331TransposeMultiply(iso->rotation, subtracted, p);
}
...
void isoGetInverseHomogeneous(const isoIsometry *iso, double homogInv[4][4]) {
    double invRotation[3][3], invTranslation[3], invTranslationNeg[3];
    mat33Transpose(iso->rotation, invRotation);
